import React from 'react'
import SubSection from '../SubSection'

function Recognition() {
  return (
    <>
    <div>
      <SubSection text={'Recognition'}/>
      <div className=' flex '>
        <div className=' w-[500px] p-[50px] ml-12'>
          <img src="./msme.jpg" alt="" />
        </div>
        <div className=' text-gray-600 text-xl p-[70px]'>
          <h1 className=' text-2xl text-blue-600 font-semibold pb-4'>Recognition of Incubator centre of Invertis University “Invertis Innovation Incubator Foundation (IIIF) by Ministry of Science & Technology, GOI</h1>
          <h2>Chitkara University Institute of Engineering & Technology has been recognized as Business Incubator (BI)/host Institution (HI) for implementation of the Scheme “Support for Entrepreneurial & management development of SMEs through incubator by Ministry of Micro, Small and Medium Enterprises, Government of India</h2>
        </div>
      </div>
    </div>
    <div>
      <div className=' flex '>
        <div className=' w-[500px] p-[50px] ml-12'>
          <img src="./sceince-tech-logo.jpg" alt="" />
        </div>
        <div className=' text-gray-600 text-xl p-[70px]'>
          <h1  className=' text-2xl text-blue-600 font-semibold pb-4'>Recognition of Incubator centre of Invertis University “Invertis Innovation Incubator Foundation (IIIF) by Ministry of Science & Technology, GOI</h1>
          <h2>Chitkara University Institute of Engineering & Technology has been recognized as Business Incubator (BI)/host Institution (HI) for implementation of the Scheme “Support for Entrepreneurial & management development of SMEs through incubator by Ministry of Micro, Small and Medium Enterprises, Government of India</h2>
        </div>
      </div>
    </div>
    
    </>

    
  )
}

export default Recognition